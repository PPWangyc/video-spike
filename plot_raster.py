...
parser.add_argument('--ref_mod', type=str, default='me', help='Input modality')
...
mod_result_all = np.load(f'{input_mod}_result.npy', allow_pickle=True).item()
...
for eid in me_result_all:
    ...
    min_bps = min(np.min(me_result['co_bps']), np.min(mod_result['co_bps']))
    max_bps = max(np.max(me_result['co_bps']), np.max(mod_result['co_bps']))
    ...
    ax_r2 = plt.subplot2grid((12, 2), (0, 0), colspan=3, fig=fig)
    ...
    ax_r2.set_xlabel(f'{ref_mod} R2')
    ax_r2.set_ylabel(f'{input_mod} R2')
    ax_r2.set_title(f'EID: {eid[:5]} \n R2 Scatter, {ref_mod} R2: {me_r2:.3f}, {input_mod} R2: {of_r2:.3f}')

    # BPS Scatter Plot, use entire row
    ax_bps = plt.subplot2grid((12, 2), (1, 0), colspan=3, fig=fig)
    ax_bps.scatter(me_result['co_bps'], mod_result['co_bps'])
    ...
    ax_bps.set_xlabel(f'{ref_mod} BPS')
    ax_bps.set_ylabel(f'{input_mod} BPS')
    ax_bps.set_title(f'BPS Scatter, {ref_mod} BPS: {me_bps:.3f}, {input_mod} BPS: {mod_bps:.3f}')
    ...
    for j, neuron_idx in enumerate(top_neuron_idx):
        ...
        of_neuron = mod_result['pred'][..., neuron_idx]
        ...
        axs_of.set_title(f'{input_mod}, R2: {of_r2_neuron[neuron_idx]:.3f}, CO-BPS: {mod_result["co_bps"][neuron_idx]:.3f}')
...
mod_bps = [eid_result[eid]['of_bps'] for eid in eid_result]
...
print(mod_bps)
print(all_bps)
ax.boxplot([me_bps, mod_bps, all_bps], labels=['ME', 'OF-2D', 'All'], medianprops=dict(color="black"))
# plot the bar plot to show the mean value
ax.bar(1, np.mean(me_bps),  label='ME',width=0.18)
ax.bar(2, np.mean(mod_bps), label='OF-2D',width=0.18)
...
ax.text(2, np.mean(mod_bps), f'{np.mean(mod_bps):.3f}', ha='center', va='bottom')
